...
for i = 1, 9 do
	globalkeys = gears.table.join(
		...
		-- Move client to tag and focus tag.
		...
		end, { description = "move focused client to tag #" .. i .. " and focus tag", group = "tag" })
	)
end
...
awesome.connect_signal("startup", function(args)
	awful.util.spawn('bash -c "rm ~/.awesome-restart || ~/.config/startup-apps.sh"')
end)
